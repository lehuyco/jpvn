...
router.get('/', async (req, res, next) => {
  ...
  try {
    if (req.user.isAdmin) {
      ...
      let companyIds = res.locals.companies.map(c => c._id)
      companyCount = companyIds.length
      query = { company: {'$in': companyIds} }
    }  
    let jobs = await Job.find(query).sort({createdAt: -1}).limit(per).skip(page*per)
    res.render('admin/jobs/index', {jobs});
  } catch (err) {
    next()
  }
});
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    res.render('admin/jobs/show', {job: job, categories})
  } catch (err) {
    next(err)
  }
});
...
router.get('/:id/delete', async (req, res, next) => {
  try {
    var result = await Job.deleteOne({_id: req.params.id})
