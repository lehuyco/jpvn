...
const UPLOAD_FOLDER= __basedir + '/public/uploads/ads/'
const UPLOAD_PATH = '/uploads/ads/'
const ADMIN_PATH = '/admin/ads/'
const VIEW_PATH = 'admin/ads/'
const ModelTitle = 'ads'
...
var createOrUpdate = async (req, res, next) => {
  let { title, subtitle, content, url, btnText, type, position } = req.body
  var doc = req.doc
  ...
  try {
    ...
    if (req.file) {
      doc.image = UPLOAD_PATH + docId + "/" + req.file.filename
      ...
      doc.thumb = UPLOAD_PATH + docId  + "/medium.png"
    }
    await doc.save()
