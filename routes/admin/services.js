...
const UPLOAD_FOLDER= __basedir + '/public/uploads/services/'
const UPLOAD_PATH = '/uploads/services/'
const ADMIN_PATH = '/admin/services/'
const VIEW_PATH = 'admin/services/'
...
var createOrUpdate = async (req, res, next) => {
  let { title, summary, content, language } = req.body
  var doc = req.doc
  ...
  try {
    ...
    if (req.file) {
      ...
      await sharp(targetDir + req.file.filename).resize(600, 400).toFile(targetDir + "medium" + ext)
