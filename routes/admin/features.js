...
const UPLOAD_FOLDER= __basedir + '/public/uploads/features/'
const UPLOAD_PATH = '/uploads/features/'
const ADMIN_PATH = '/admin/features/'
const VIEW_PATH = 'admin/features/'
const ModelTitle = 'features'
...
var createOrUpdate = async (req, res, next) => {
  let { title, content, language } = req.body
  var doc = req.doc
  ...
  try {
    ...
    if (req.file) {
      ...
      doc.image = UPLOAD_PATH + docId + "/" + req.file.filename + '?at=' + timestamp
      var targetDir = UPLOAD_FOLDER + docId + "/"
      await sharp(targetDir + req.file.filename).resize(300, 200).toFile(targetDir + "medium" + ext)
      doc.thumb = UPLOAD_PATH + docId  + "/medium" + ext + '?at=' + timestamp
    }
    await doc.save()
...
router.get('/', async (req, res, next) => {
  ...
  try {
    ...
    res.render(VIEW_PATH + 'index', { docs: data.docs, page, total: data.pages});
  } catch (err) {
    next(err)
  }
});
