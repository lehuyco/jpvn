...
var updatePost = async (req, res, next) => {
  let { title, content, summary, category, publishedAt, wpslug, status, categories, keywords } = req.body
  var post = req.post
  ...
  try {
    ...
    if (req.file) {
      ...
      post.image = '/uploads/posts/' + postId + "/" + req.file.filename + '?at=' + timestamp
      var targetDir = POST_UPLOAD_PATH + postId + "/"
      await sharp(targetDir + req.file.filename).resize(300, 200).toFile(targetDir + "medium.png")
      post.thumb = '/uploads/posts/' + postId  + '/medium.png' + '?at=' + timestamp
    }
    await post.save()
...
router.get('/', async (req, res, next) => {
  ...
  try {
    ...
    var data = await Post.paginate(query, { page: page, limit: 20, sort: { createdAt: -1 }})
    res.render('admin/posts/index', { posts: data.docs, page, total: data.pages});
  } catch (err) {
    next(err)
  }
});
