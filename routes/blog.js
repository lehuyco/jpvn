...
router.get('/news', async (req, res, next) => {
  ...
  try {
    var data = await Post.paginate({ categories: { $in: res.locals.categorieIds } }, { sort: { createdAt: -1 }, populate: 'categories', page: page, limit: 10})
    
    res.locals.title = __('menu.blog')
    res.locals.ogUrl = __host + '/blog'
    
    res.render('blog/index', { posts: data.docs, page, total: data.pages});
  } catch (err) {
    next(err)
  }
});
...
router.get('/cat/:slug', async (req, res, next) => {
  ...
  try {
    ...
    let data = await Post.paginate({categories: { $in: [category._id] }}, { sort: { createdAt: -1 }, populate: 'categories', page: page, limit: 10})
    
    res.locals.title = category.title
    res.locals.ogUrl = __host + '/' + category.slug

    res.render('blog/index', { title, category, posts: data.docs, page, total: data.pages});
  } catch (err) {
    next(err)
  }
});
...
router.get('/post/:slug', async (req, res, next) => {
  try {
    let post = await Post.findOne({slug: req.params.slug}).populate('categories')
